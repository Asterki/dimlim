...
const addContact = async (userID: string, contactUsername: string) => {
  try {
    ...
    const contact: HydratedDocument<User> | null = await UserModel.findOne({ 'profile.username': contactUsername.toLowerCase() }).populate('contacts');
...
const removeContact = async (userID: string, contactUsername: string) => {
  try {
    ...
    const contact: HydratedDocument<User> | null =  await UserModel.findOne({ 'profile.username': contactUsername.toLowerCase() });
    if (!contact) return 'contact-not-found';
...
export { addContact, removeContact, blockContact, unblockContact, getContacts, getProfile, acceptContact };
